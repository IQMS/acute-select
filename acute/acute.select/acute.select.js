    ...
    return {
      ...
      controller: function($scope, $element, $window, $rootScope, $timeout, $filter, navKey, safeApply) {
        ...
        $scope.getItemClass = function(i) {
          if ($scope.selectedItem && matches($scope.items[i].value, $scope.selectedItem.value)) {
        ...
        function confirmSelection(item, forceClose) {
          ...
          //prevent close if forceClose is false, but ignore if forceClose is undefined
          if (angular.isDefined(forceClose)) close = forceClose;
          if ($scope.popupVisible && close) {
            $scope.popupVisible = false;
